...
def plot_celestial_summary(
    ...
    # Compactness at G=1 vs G×10^36 (gravity=EM threshold)
    c1 = np.array([body_compactness(masses[i], radii_std[i], constants.G, constants.c) for i in range(len(bodies))])
    c1e36 = np.array([body_compactness(masses[i], body_radius_scaled(radii_std[i], 1e36), constants.G * 1e36, constants.c) for i in range(len(bodies))])
    yp = np.arange(len(bodies))
    bh = 0.35
    ax2.barh(yp - bh/2, c1, height=bh, color=COLORS['standard'], edgecolor='black', alpha=0.8, label='G = G₀')
    ax2.barh(yp + bh/2, c1e36, height=bh, color=COLORS['scaled'], edgecolor='black', alpha=0.8, label='G = 10³⁶ G₀')
    ax2.set_yticks(yp)
    ax2.set_yticklabels(nd, fontsize=11)
    ax2.set_xscale('log')
    ax2.set_xlabel('Kompaktheit Rₛ / R' if de else 'Compactness Rₛ / R', fontsize=12)
    ax2.set_title('2. ' + ('Kompaktheit bei G₀ vs. 10³⁶ G₀ (Gravitation=EM)' if de else 'Compactness at G₀ vs. 10³⁶ G₀ (Gravity=EM)'), fontsize=14, fontweight='bold', pad=15)
    ...
    G_sc = np.logspace(0, 40, 400)  # Extended to 10^40 to show full range including 10^36 threshold
    jm = np.array([jeans_mass_at_scale(gs, 20.0, 1e-18, constants) for gs in G_sc])
    ax3.loglog(G_sc, jm / constants.M_sun, color=COLORS['primary_blue'], linewidth=2.5, label='M_J (T=20 K)')
    for i in range(len(bodies)):
